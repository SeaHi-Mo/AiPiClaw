...
static void register_tool(const mimi_tool_t *tool)
{
    if (s_tool_count >= MAX_TOOLS) {
        AXK_LOG_ERROR("cron", "Tool registry full");
        return;
    }
    s_tools[s_tool_count++] = *tool;
    AXK_LOG_INFO("cron", "Registered tool: %s", tool->name);
}
...
static void build_tools_json(void)
{
    ...
    AXK_LOG_INFO("cron", "Tools JSON built (%d tools)", s_tool_count);
}
...
int axk_tool_registry_init(void)
{
    ...
    AXK_LOG_INFO("cron", "Tool registry initialized");
    return 0;
}
                                ...
                                char *output, size_t output_size)
{
    for (int i = 0; i < s_tool_count; i++) {
        if (strcmp(s_tools[i].name, name) == 0) {
            AXK_LOG_INFO("cron", "Executing tool: %s", name);
            return s_tools[i].execute(input_json, output, output_size);
        }
    }

    AXK_LOG_WARN("cron", "Unknown tool: %s", name);
